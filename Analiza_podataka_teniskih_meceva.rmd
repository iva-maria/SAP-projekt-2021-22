...
```{r}
library(rvest)
strania <- read_html("http://www.tennisdrawchallenge.com/data/list/one-handed-backhand")
tables <- strania %>% html_table(fill = TRUE)
jedno_back <- tables[[1]]
igraci_1HBH <- jedno_back["Name"]
```

```{r}
"Denis Shapovalov" %in% igraci_1HBH[["Name"]]
```
