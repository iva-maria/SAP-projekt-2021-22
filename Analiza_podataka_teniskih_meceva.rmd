...
###Test jednakosti varijanci dviju varijabli
...
2)Jesu li ljevaci nezgodniji protivnici dešnjacima koji igraju jednoručni backhand?

Malo ćemo izmjeniti pitanje. Gledat ćemo jesu li dešnjaci nezgodniji protivnici od ljevaka.
Postavljamo nultu hipotezu: jednako je zahtjevno dešnjacima s jednoručnim backhandom igrati protiv dešnjaka i ljevaka. Alternativna hipoteza će nam biti da su dešnjaci nezgodniji protivnici.
Jesu li protivnici zahtjevni, lagani, nezgodniji i slično ćemo promatrati po broju pobjeda naspram odigranim mečevima. Tu imamo dva uzorka i oba uzorka će biti ispunjena isključivo sa dva podatka, pobjeda ili poraz.
...
Sada ćemo nad tim podacima napraviti Test o dvije proporcija.

H0 - Igrači koji igraju desnim jednoručnim backhandom jednako dobro igraju protiv dešnjaka i ljevaka. p1=p2
H1 - Igračima s desnim jednoručnim backhandom su dešnjaci nezgodniji protivnici od ljevaka p1<p2
...
k1,k2 su broj pobjeda
n1,n2 su broj mečeva
Z-statistika: 
$$ \begin{aligned}
  Z = \frac{\frac{k1}{n1}- \frac{k2}{n2}}{\sqrt{p(1-p) (\frac{1}{n1}+\frac{1}{n2})}} & \sim N(0, 1) \\
\end{aligned} $$
Sada slijedi računanje z_vrijednosti i p-vrijednosti
```{r}
z_vrijednost = (k1/n1 - k2/n2)/sqrt(((k1+k2)/(n1+n2))*(1-(k1+k2)/(n1+n2))*(1/k1+1/k2))
z_vrijednost
pnorm(z_vrijednost)
```
p-vrijednosti je manja od 0.01
Možemo odbaciti nultu hipotezu u korist prve hipoteze na razini značajnosti $\alpha$ = 0.01
Zaključujemo da su igračima s desnim jednoručnim backhandom nezgodniji protivnici dešnjaci

```{r}
prop.test(c(k1,k2), c(n1,n2), p = NULL,
          alternative = "less",
...
pnorm(grupiraneruke$Freq[1]/nrow(razl_ruke), mean = nrow(razl_ruke) * 0.5, sd = sqrt(nrow(razl_ruke) * 0.25), lower.tail = TRUE)
...
U nastavku promatramo samo best of 3 mečeve.
...
```
Vidimo da postotak takvih mečeva iznosi otprilike 82%. p-vrijednost je praktički 0.
Odbacujemo H0 na jako velikoj razini značajnosti i zaključujemo da su dobitnici prvog seta u prosjeku bolji igrači (imaju veću vjerojatnost dobitka pojedinog seta).
...
Zelenom bojom je prikazan višak opaženih u odnosu na očekivane mečeve. Razlika je manja od 5000 mečeva, ali je s danim parametrima distribucije i više nego dovoljna da opovrgne H0.

Promotrimo vezu između razlike u rangu i trajanja meča.
Pretpostavka je da igrači sličnijeg ranga igraju dulje mečeve.
...
Kao što vidimo, dobivamo vrlo mali R^2 te ne možemo zaključiti da su varijable linearno zavisne. Preduvjeti nisu bili dobri, a prividan padajući trend možemo objasniti i brojem odigranih mečeva u kojima su igrači imali sličan rang. Naime, veći ekstremi se prirodno postižu na mjestima brojnijeg uzorkovanja.
Na grafu možemo uočiti gornju envelopu koja uistinu ima padajući trend, ali također postoji i donja envelopa koja nema takav trend. Mečevi blizu gornje envelope su rijetki i ne utječu toliko na regresiju koliko utječu oni drugi, čisto zbog svoje brojnosti.

Da bi se uvjerili da je ovaj prividan padajući trend uistinu rezultat neravnomjernoj raspoređenosti podataka po horizontalnoj osi, razmotrimo sljedeći graf:
```{r}
hist(known_ranks$rank_dif)
```

Graf potvrđuje tu teoriju.
